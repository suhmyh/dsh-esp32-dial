...
void DialUi::buildMainScreen() {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    lv_obj_set_pos(idleColLeft_[i], 45, rowY[i]);
    lv_obj_set_width(idleColLeft_[i], 100);
    ...
    lv_obj_set_pos(idleColRight_[i], 215, rowY[i]);
    lv_obj_set_width(idleColRight_[i], 100);
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    lv_obj_set_pos(idleNameLeft_[i], 45, rowY[i] - 18);
    lv_obj_set_width(idleNameLeft_[i], 100);
    ...
    lv_obj_set_pos(idleNameRight_[i], 215, rowY[i] - 18);
    lv_obj_set_width(idleNameRight_[i], 100);
...
void DialUi::setState(const JsonDocument& doc) {
  PetState state;
  petStateFromJson(doc, state);
  setState(state, doc);
}

void DialUi::setState(const PetState& state, const JsonDocument& doc) {
  ...
  const DialPhase newPhase = static_cast<DialPhase>(state.phase);
  ...
  uint8_t ctx = state.contextPercent;
  updateArc(ctx);
  ...
  if (strlen(state.title) > 0) lv_label_set_text(titleLabel_, state.title);
  lv_label_set_text(detailLabel_, state.detail);
  ...
  if (idleFace) {
    if (strlen(state.clock) > 0) lv_label_set_text(clockLabel_, state.clock);
  ...
  if (idleFace && ctxLabel_ != nullptr) {
    const uint8_t ctx = state.contextPercent;
    if (ctx > 0) {
      char buf[24];
...
void DialUi::backgroundDoubleClicked(lv_event_t* /*event*/) {
  // LVGL 8.4 exposes short-click events but not a double-click event. AppShell
  // owns the time window so the gesture remains part of navigation rather than
  // part of the Codex renderer.
  appShell.onCodexBackgroundTap();
}
